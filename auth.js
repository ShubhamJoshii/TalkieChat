...
router.post("/saveChatID",Authenication,async (req,res)=>{
  ...
  if(chatIDExists){
    ...
    const saveChatID = await ChatDataModel({ChatID,User1_id:req.userID,User1_Name:req.rootUser.Name,User1_Avatar:req.rootUser.Avatar,User1_AvatarBackground:req.rootUser.AvatarBackground});
    saveChatID.save();
...
router.get("/chattingData",Authenication,async(req,res)=>{
  try{
    const fetchUserID = await ChatDataModel.find({$or:[{User1_id:req.userID},{User2_id:req.userID}]});
